...
/* Hardware gallery · full-bleed image hero · product gallery + prominent spec table (theme via shell vars) */
export function HardwareGalleryTemplate({ product, category, related }: TemplateProps) {
  const paras = product.description.split("\n").filter(Boolean);
  const base = `/images/products/${product.slug}`;
  const gallery = [
    { src: product.heroImage, label: product.name },
    { src: `${base}/a.jpg`, label: `${product.name} detail` },
    { src: `${base}/b.jpg`, label: `${product.name} in use` },
  ];

  return (
    <article>
      {/* Hero — full-bleed image, content lower-left */}
      <section className="relative flex min-h-[82vh] items-end overflow-hidden bg-[var(--ink)]">
        ...
        <Container className="relative z-10 pb-16 pt-40 md:pb-24">
          <div className="max-w-3xl text-white">
            <p className="animate-rise mb-5 inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--acc)]">
              {product.productType === "Hardware" ? <Printer className="h-3.5 w-3.5" /> : <Cpu className="h-3.5 w-3.5" />}
      ...
      {/* Signature — product gallery */}
      <Section padding="none" className="border-y border-border bg-surface py-20 md:py-28">
        <Container>
          <div data-reveal className="mb-12 max-w-2xl">
            <p className="mb-3 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--acc)]">Gallery</p>
            <h2 className="text-navy">A closer look</h2>
          </div>
          <div data-reveal className="grid gap-4 md:grid-cols-3 md:grid-rows-2">
            <div className="overflow-hidden border border-border bg-[var(--ink)] md:col-span-2 md:row-span-2">
              <div className="aspect-[16/11] h-full w-full md:aspect-auto">
                <SmartImage src={gallery[0].src} alt={gallery[0].label} label={gallery[0].label} priority className="transition-transform duration-700 ease-out hover:scale-105" />
              </div>
            </div>
            {gallery.slice(1).map((g, i) => (
              <div key={i} className="overflow-hidden border border-border bg-[var(--ink)]">
                <div className="aspect-[16/11] w-full">
                  <SmartImage src={g.src} alt={g.label} label={g.label} className="transition-transform duration-700 ease-out hover:scale-105" />
                </div>
              </div>
            ))}
          </div>
        ...
        <Section padding="none" className="py-24 md:py-32">
          <Container>
            <div data-reveal className="mb-14 max-w-2xl">
              ...
              <h2 className="text-navy">Built for the counter</h2>
            </div>
            <div data-reveal className="grid gap-px overflow-hidden border border-border bg-border sm:grid-cols-2 lg:grid-cols-3">
              {product.keyFeatures.map((f, i) => (
                <div key={i} className="bg-surface p-8">
                  <span className="mb-5 inline-flex h-12 w-12 items-center justify-center rounded-full bg-[var(--acc)]/15 text-[var(--ink)]"><Zap className="h-5 w-5" /></span>
      ...
      {/* Specifications — centrepiece spec table */}
            ...
            <div data-reveal className="grid gap-12 lg:grid-cols-[0.4fr_0.6fr] lg:gap-16">
              <div className="lg:sticky lg:top-24 lg:h-fit">
                ...
                {product.deploymentType && (
                  <p className="mt-5 text-text-muted">Supplied, installed and configured — {product.deploymentType}.</p>
                )}
              </div>
      ...
      <CtaBanner heading={`Need ${product.name}?`} text="Tell us your requirements and our team will recommend, supply and install the right hardware — with setup and support included." image={`/images/products/${product.slug}/a.jpg`} />
    </article>
  );
}
